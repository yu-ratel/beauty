  ...
  return (
    <main className="h-[80%]">
      ...
      <section className="m-10 h-4/5 bg-white">
        <ol className="flex h-[10%] border-b-[1px] bg-deepBraun text-center text-white *:mt-3 *:w-[25%] max-md:*:w-[35%]">
          {titles.map((title, idx) => {
            return (
              <li key={title} className={idx === responsivePopIndex ? 'hidden md:block' : ''}>
                {title}
              </li>
            );
          })}
